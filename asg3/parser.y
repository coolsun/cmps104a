...
structdef : struct_root '}'        { free_ast($2); $$ = $1; }
          ;

struct_root: struct_root fielddecl ';'
...
fielddecl : basetype TOK_IDENT    { $$ = adopt1sym ($1,$2,TOK_FIELD);}
          | basetype TOK_NEWARRAY TOK_IDENT
                                  { $$=adopt2(change_sys($2,TOK_ARRAY),
                                    $1, change_sys($3, TOK_FIELD));}
          ;
...
basetype  : TOK_VOID              { $$ = $1; }
          ...
          | TOK_INT               { $$ = $1; }
          | TOK_STRING            { $$ = $1; }
          | TOK_IDENT             { $$=change_sys($1, TOK_TYPEID);}
          ;
...
function  : identdecl '(' ')' block    
                             { free_ast($3); 
                               ...
                               $$ = adopt1(root,$1);
                               $$ = adopt1sym(root,$2,TOK_PARAMLIST); 
                               }
...
identdecl : basetype TOK_IDENT    { $$ = adopt1sym ($1,$2,TOK_DECLID);}
          | basetype TOK_NEWARRAY TOK_IDENT
                             { $$ = adopt2 (change_sys($2, TOK_ARRAY),
                               $1, change_sys($3, TOK_DECLID));}
          ;

block     : block_root '}'         { free_ast($2); $$ = $1; }
          | ';'                   { $$ = $1; }
          ;
 
block_root : block_root statement   { $$ = adopt1($1, $2); }
          | '{'                   { $$ = change_sys($1, TOK_BLOCK);}
          ;
...
return    : TOK_RETURN ';'        { $$=change_sys($1,TOK_RETURNVOID);}
...
call      : TOK_IDENT '('')'      { free_ast($3); 
          ...
          | call_root ')'          { free_ast($2); $$ = $1; }
          ;

call_root  : call_root ',' expr     { free_ast($2); adopt1($1,$3); }
          | TOK_IDENT '(' expr    { $$ = adopt2(change_sys($2,TOK_CALL),
                                    $1, $3); }

variable  : TOK_IDENT             { $$ = $1; }
          | expr '[' expr ']' %prec '['   
                                  { free_ast($4); 
                                    $$=adopt2(change_sys($2,TOK_INDEX),
                                    $1,$3); }
          | expr '.' TOK_IDENT  
                                  { $$ = adopt2($2, $1, 
                                    change_sys($3, TOK_FIELD)); } 
          ;
