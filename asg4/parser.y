...
%token TOK_POS TOK_NEGTOK_NEWARRAY TOK_TYPEID TOK_FIELD
...
declgroup : declgroup d1 ';'     { $$ = adopt1($1,$2); }
          |                         { $$ = new_astree(struct_,0,0,0,""); } 
          ;

d1     : type1 TOK_IDENT     { $$ = adopt2(new_astree(decl,0,0,0,""), $1, $2); }
          ;

type1   : base_type TOK_ARRAY    { $$ = adopt2(new_astree(type,0,0,0,""), $1, $2); }
        | base_type              { $$ = adopt1(new_astree(type,0,0,0,""), $1); }
        ;

base_type : TOK_VOID             { $$ = adopt1(new_astree(basetype,0,0,0,""), $1); }
...
comdecl  :  comdecl ',' d1                       { $$ = adopt1($1, $3); } 
         |  d1                                   { $$ = adopt1(new_astree(function,0,0,0,""), $1); }
         ;
...
block_root : block_root statement {$$ = adopt1($1,$2); }
          |                     {$$ = new_astree(block,0,0,0,""); }
          ;
...
allocator1 : TOK_NEW base_type '(' expr ')'   { $$ = adopt1(adopt2(new_astree(allocator_,0,0,0,""),$1,$2),$4); }
           | TOK_NEW base_type '(' ')'        { $$ = adopt2(new_astree(allocator_,0,0,0,""),$1,$2); }
           | TOK_NEW base_type '[' expr ']'   { $$ = adopt1(adopt2(new_astree(allocator_,0,0,0,""),$1,$2),$4); }
           ;
